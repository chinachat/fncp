...
打开 Web 终端后，点击右上角 **⚙ 设置** 可配置以下防护（保存在服务器 `config.json`，即时生效，首次运行自动生成）：

| 防护项 | 说明 | 示例 |
|--------|------|------|
| 访问密码 | 开启后必须输入密码才能打开终端；密码以 PBKDF2-SHA256 哈希存储，登录态为 24 小时 HttpOnly Cookie | 任意强密码 |
...
> 首次安装后默认不启用密码（保证可进入设置页）；强烈建议第一时间设置访问密码。
> 信任网段/网址留空 = 不限制；填错网段导致把自己挡在外面时，可编辑 `config.json` 或直接删除该文件恢复默认。
